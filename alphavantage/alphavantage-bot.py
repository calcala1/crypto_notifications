...
SYMBOL = random.choice(
    ['MSFT', 'AAPL', 'AMZN', 'GOOGL', 'FB', 'INTC', 'CSCO', 'CMCSA', 'PEP', 'ADBE', 'NVDA', 'NFLX', 'PYPL', 'COST',
     'SBUX', 'QCOM', 'INTU', 'TMUS'])
# Stock Symbol
    ...
    if CHARTDOMAIN == 'stock':
        ...
        ax.set_title(
            "Latest Data -> OPEN : {} | HIGH : {} | LOW : {} | CLOSE : {} \n\n {} {} value chart of {} - Last updated at {}"
                .format(values['1. open'][0], values['2. high'][0], values['3. low'][0], values['4. close'][0],
                        REQ_TYPE, PLOT, SYMBOL, now)
        )
        ...
        if crypto_market == 'CNY':
            ax.set_title(
                "Latest Data({} market) -> OPEN : {} | HIGH : {} | LOW : {} | CLOSE : {} \n\n {} {} value chart of {} - Last updated at {}"
                    .format(crypto_market, values['1a. open (CNY)'][0], values['2a. high (CNY)'][0],
                            values['3a. low (CNY)'][0], values['4a. close (CNY)'][0], CRYPTOINTERVAL, CRYPTOPLOT,
                            CRYPTOSYMBOL, now)
            )
        if crypto_market == 'USD':
            ax.set_title(
                "Latest Data({} market) -> OPEN : {} | HIGH : {} | LOW : {} | CLOSE : {} \n\n {} {} value chart of {} - Last updated at {}"
                    .format(crypto_market, values['1b. open (USD)'][0], values['2b. high (USD)'][0],
                            values['3b. low (USD)'][0], values['4b. close (USD)'][0], CRYPTOINTERVAL, CRYPTOPLOT,
                            CRYPTOSYMBOL, now)
            )
        values[plot].plot(color=COLOR)
        plt.ylabel('Price')
...
def rewrite_readme():
    ...
    now = datetime.now().strftime("%b %d, %Y(%H:%M:%S)")
    promote_text = "**Realtime Stock/Crytpocurrency Chart📈  Rendered By [AlphaVantage-Action-Bot](https://github.com/manojnaidu619/AlphaVantage-Action-Bot) | Last updated the above chart on {} **".format(
        now)
